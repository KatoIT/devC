...
void xuat(int n,tinh *a,FILE *f=stdout)
{
	...
	for(int i=0;i<n;i++) 
	fprintf(f,"%-30s%-30s%-15.3f%d\n",a[i].ten,a[i].khuvuc,a[i].dientich,a[i].danso);
}
...
{
	int d=0;
	for(int i=0;i<n;i++) d+=a[i].danso>=u&&a[i].danso<v;
	return d;
}
...
int main()
{
	...
	for(int i=1;i<=4;i++) printf("\nSo tinh co dan so trong trong [%d %d) la %d",tk[i-1],tk[i],dem(n,a,tk[i-1],tk[i]));
	tim(n,a);
	dientichmax(n,a);
	free(a);
	fclose(f);
